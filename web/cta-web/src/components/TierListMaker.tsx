...
export function TierListMaker() {
  ...
  const [heroes, setHeroes] = useState<Hero[]>([]), [selected, setSelected] = useState<string>()
  ...
  const reset = () => { setTitle(defaultTitle); setTiers(initialTiers); setAssignments({}); setSelected(undefined) }
  ...
  const selectHero = (heroId: string) => setSelected(current => current === heroId ? undefined : heroId)
  ...
  return <main className="tier-page"><button className="back" onClick={() => navigate('/')}>← Hero library</button>
    ...
    {error ? <ErrorState message={error} retry={() => setAttempt(value => value + 1)} /> : loading ? <LoadingState>Loading portraits…</LoadingState> : <div className="tier-workspace">
      <section className="tier-board" aria-label="Hero tiers">{tiers.map((tier, index) => <TierRow key={tier.id} tier={tier} color={tierColors[index]} heroes={heroesFor(tier.id)} selected={selected} select={selectHero} place={place} rename={rename} remove={tiers.length > 1 ? () => deleteTier(tier.id) : undefined} moveUp={index > 0 ? () => moveTier(tier.id, -1) : undefined} moveDown={index < tiers.length - 1 ? () => moveTier(tier.id, 1) : undefined} />)}</section>
      <section className="unranked" aria-labelledby="unranked-title" onDragOver={event => event.preventDefault()} onDrop={event => place(event.dataTransfer.getData('text/plain'))}>
        <div className="unranked-heading"><h2 id="unranked-title">Unranked <small>{heroesFor().length}</small></h2>{selected && assignments[selected] && <button onClick={() => place(selected)}>Move selected here</button>}</div>
        <PortraitPool heroes={heroesFor()} selected={selected} select={selectHero} />
      </section>
    </div>}
  </main>
}

function TierRow({ tier, color, heroes, selected, select, place, rename, remove, moveUp, moveDown }: { tier: Tier; color: string; heroes: Hero[]; selected?: string; select: (id: string) => void; place: (hero: string, tier: string) => void; rename: (id: string, name: string) => void; remove?: () => void; moveUp?: () => void; moveDown?: () => void }) {
  return <section className="tier-row" aria-label={`${tier.name || 'Unnamed'} tier`} onDragOver={event => event.preventDefault()} onDrop={event => place(event.dataTransfer.getData('text/plain'), tier.id)}>
    ...
    <PortraitPool heroes={heroes} selected={selected} select={select} />
    <div className="tier-controls">{selected && <button onClick={() => place(selected, tier.id)}>Place here</button>}<div className="tier-order">{moveUp && <button aria-label={`Move ${tier.name || 'unnamed'} tier up`} onClick={moveUp}>↑</button>}{moveDown && <button aria-label={`Move ${tier.name || 'unnamed'} tier down`} onClick={moveDown}>↓</button>}</div>{remove && <button aria-label={`Delete ${tier.name || 'unnamed'} tier`} onClick={remove}>Delete</button>}</div>
  </section>
}

function PortraitPool({ heroes, selected, select }: { heroes: Hero[]; selected?: string; select: (id: string) => void }) {
  return <div className="portrait-pool">{heroes.map(hero => <button key={hero.id} className={`tier-portrait ${selected === hero.id ? 'selected' : ''}`} aria-pressed={selected === hero.id} aria-label={`Select ${hero.name}`} title={hero.name} draggable onDragStart={event => event.dataTransfer.setData('text/plain', hero.id)} onClick={() => select(hero.id)}><Portrait hero={hero} /><span className="tier-portrait-name">{hero.name}</span></button>)}</div>
}

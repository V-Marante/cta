...
describe('hero symbols', () => {
  it.each(['Brawler', 'Barbarian', 'Knight', 'Rogue', 'Lancer', 'Samurai', 'Ranger', 'Magician', 'Gunner', 'Support'])('uses an authentic local %s job icon URL', job => {
    ...
    expect(screen.getByLabelText(job).querySelector('img')).toHaveAttribute('src', expect.stringContaining(`/assets/ui-icons/synthetic/jobs/${job.toLowerCase()}.png`))
  })
  it.each(['Fire', 'Water', 'Earth', 'Light', 'Dark'])('uses an authentic local %s element icon URL', element => {
    render(<ElementIcon element={element} />)
    expect(screen.getByLabelText(element).querySelector('img')).toHaveAttribute('src', expect.stringContaining(`/assets/ui-icons/synthetic/elements/${element.toLowerCase()}.png`))
  })

...
public static class HeroEndpoints
{
    ...
    {
        var importId = await repository.LatestImportAsync();
        var heroes = importId is null ? [] : await repository.LoadHeroesAsync(importId);
    ...
    {
        var importId = await repository.LatestImportAsync();
        return importId is null ? Results.NotFound() : Results.Ok(await repository.LoadSkillsAsync(importId, id));
    }
